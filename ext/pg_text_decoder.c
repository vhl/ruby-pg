...
VALUE rb_cPG_TimestampWithTimeZone;
...
static VALUE pg_text_dec_timestamp_with_time_zone_do(VALUE rstr)
{
  const char *str;
  int len;
  ...
  )
  {
    ...
    if (time != -1)
    {
      ...
      res = rb_time_timespec_new(&ts, gmt_offset);
      return res;
    }
  }
  return rstr; 
...
pg_text_dec_timestamp_with_time_zone(int argc, VALUE *argv, VALUE self)
{
  ...
  return pg_text_dec_timestamp_with_time_zone_do(argv[0]);
}
